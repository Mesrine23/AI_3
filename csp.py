...
def dom_wdeg(assignment, csp):
    ...
    total_weight = 0
    for neighb in csp.constrained_neighbors:
        if(neighb[0] not in assignment and neighb[1] not in assignment):
            total_weight += csp.weight[neighb]

    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: domain_weight_degree_ratio(csp, var, assignment,total_weight))

def domain_weight_degree_ratio(csp, subject, assignement, total_weight):
    if (csp.curr_domains != None):
        return (len(csp.curr_domains[subject]) / max(1,total_weight))
    else:
        return (len(csp.domains[subject]) / max(1,total_weight))
# Value ordering
